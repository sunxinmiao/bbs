...
public class CosMultipartRequest {
	 ...
	 */
	private static final Logger logger = Logger.getLogger(CosMultipartRequest.class);
  ...
  public Enumeration<String> getParameterNames() {
		...
		Enumeration<String> returnEnumeration = parameters.keys();
		if (logger.isDebugEnabled()) {
  ...
  public Set<String> getFileNames() {
		if (logger.isDebugEnabled()) {
			logger.debug("getFileNames() - start"); //$NON-NLS-1$
		}

		Set<String> returnSet = files.keySet();
		if (logger.isDebugEnabled()) {
			logger.debug("getFileNames() - end"); //$NON-NLS-1$
		}
    return returnSet;
  }
    ...
    try {
      ...
      if (values == null || values.size() == 0) {
        ...
        return null;
			...
			String returnString = values.get(values.size() - 1);
			if (logger.isDebugEnabled()) {
				logger.debug("getParameter(String) - end"); //$NON-NLS-1$
			}
      return returnString;
			...
			logger.error("getParameter(String)", e); //$NON-NLS-1$
  ...
  public String[] getParameterValues(String name) {
    ...
    if (values == null || values.size() == 0) {
      ...
      return null;
			...
			String[] returnStringArray = values.toArray(new String[values.size()]);
			if (logger.isDebugEnabled()) {
				logger.debug("getParameterValues(String) - end"); //$NON-NLS-1$
			}
  ...
  public String getFilesystemName(String name) {
    ...
    try {
      UploadedFile file = files.get(name);
			String returnString = file.getFilesystemName();
			if (logger.isDebugEnabled()) {
				logger.debug("getFilesystemName(String) - end"); //$NON-NLS-1$
			}
      return returnString;  // may be null
    }
    catch (Exception e) {
			logger.error("getFilesystemName(String)", e); //$NON-NLS-1$
  ...
  public String getOriginalFileName(String name) {
		if (logger.isDebugEnabled()) {
			logger.debug("getOriginalFileName(String) - start"); //$NON-NLS-1$
		}

    try {
      UploadedFile file = (UploadedFile)files.get(name);
			String returnString = file.getOriginalFileName();
			if (logger.isDebugEnabled()) {
				logger.debug("getOriginalFileName(String) - end"); //$NON-NLS-1$
			}
      return returnString;  // may be null
    }
    catch (Exception e) {
			logger.error("getOriginalFileName(String)", e); //$NON-NLS-1$
  ...
  public String getContentType(String name) {
    ...
    try {
      UploadedFile file = (UploadedFile)files.get(name);
			String returnString = file.getContentType();
			if (logger.isDebugEnabled()) {
      ...
      return returnString;  // may be null
			...
			logger.error("getContentType(String)", e); //$NON-NLS-1$
  ...
  public File getFile(String name) {
    ...
    try {
      ...
      return returnFile;  // may be null
    }
    catch (Exception e) {
			logger.error("getFile(String)", e); //$NON-NLS-1$
...
class UploadedFile {
  ...
  public File getFile() {
		if (logger.isDebugEnabled()) {
			logger.debug("getFile() - start"); //$NON-NLS-1$
		}

    if (dir == null || filename == null) {
			if (logger.isDebugEnabled()) {
				logger.debug("getFile() - end"); //$NON-NLS-1$
			}
      return null;
    }
    else {
			File returnFile = new File(dir + File.separator + filename);
			if (logger.isDebugEnabled()) {
				logger.debug("getFile() - end"); //$NON-NLS-1$
			}
      return returnFile;
