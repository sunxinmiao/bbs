...
public class BufferedServletInputStream extends ServletInputStream {
	 ...
	 */
	private static final Logger logger = Logger.getLogger(BufferedServletInputStream.class);
  ...
  public int readLine(byte b[], int off, int len) throws IOException {
		if (logger.isDebugEnabled()) {
			logger.debug("readLine(byte[], int, int) - start"); //$NON-NLS-1$
		}

    int total = 0;
    if (len == 0) {
			if (logger.isDebugEnabled()) {
				logger.debug("readLine(byte[], int, int) - end"); //$NON-NLS-1$
			}
      return 0;
    }
  ...
  private static int findeol(byte b[], int pos, int len) {
		if (logger.isDebugEnabled()) {
			logger.debug("findeol(byte[], int, int) - start"); //$NON-NLS-1$
		}
    ...
    while (i < end) {
      if (b[i++] == '\n') {
				int returnint = i - pos;
				if (logger.isDebugEnabled()) {
					logger.debug("findeol(byte[], int, int) - end"); //$NON-NLS-1$
				}
        return returnint;
      }
    }
