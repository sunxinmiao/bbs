...
public class MultipartParser {
	 ...
	 */
	private static final Logger logger = Logger.getLogger(MultipartParser.class);
  ...
  public Part readNextPart() throws IOException {
		if (logger.isDebugEnabled()) {
			logger.debug("readNextPart() - start"); //$NON-NLS-1$
		}
    ...
    if (line == null) {
      // No parts left, we're done
		...
		if (logger.isDebugEnabled()) {
				logger.debug("readNextPart() - end"); //$NON-NLS-1$
			}
      return null;
    }
    ...
    if (filename == null) {
      ...
      // The encoding is needed to help parse the value
			Part returnPart = new ParamPart(name, in, boundary, encoding);
			if (logger.isDebugEnabled()) {
				logger.debug("readNextPart() - end"); //$NON-NLS-1$
			}
      return returnPart;
  ...
  private String extractBoundary(String line) {
		if (logger.isDebugEnabled()) {
			logger.debug("extractBoundary(String) - start"); //$NON-NLS-1$
		}
    ...
    if (index == -1) {
			if (logger.isDebugEnabled()) {
				logger.debug("extractBoundary(String) - end"); //$NON-NLS-1$
			}
      return null;
    }
  ...
  private String[] extractDispositionInfo(String line) throws IOException {
		if (logger.isDebugEnabled()) {
			logger.debug("extractDispositionInfo(String) - start"); //$NON-NLS-1$
		}
	...
	if (logger.isDebugEnabled()) {
			logger.debug("extractDispositionInfo(String) - end"); //$NON-NLS-1$
		}
    return retval;
  }
  ...
  private static String extractContentType(String line) throws IOException {
		if (logger.isDebugEnabled()) {
			logger.debug("extractContentType(String) - start"); //$NON-NLS-1$
		}
    ...
    if (end == -1) {
		...
		String returnString = line.substring(13, end).trim();
		if (logger.isDebugEnabled()) {
			logger.debug("extractContentType(String) - end"); //$NON-NLS-1$
		}
    return returnString;  // "content-type:" is 13
  }
