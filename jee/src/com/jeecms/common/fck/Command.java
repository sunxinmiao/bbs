 ...
 *</ul>
 * 
 * @version $Id: Command.java,v 1.1 2011/12/26 03:47:53 Administrator Exp $
 */
public class Command {
	...
	public static boolean isValidForGet(final String name) {
		if (logger.isDebugEnabled()) {
			logger.debug("isValidForGet(String) - start"); //$NON-NLS-1$
		}

		boolean returnboolean = getCommands.containsKey(name);
		if (logger.isDebugEnabled()) {
			logger.debug("isValidForGet(String) - end"); //$NON-NLS-1$
		}
	...
	public static boolean isValidForPost(final String name) {
		if (logger.isDebugEnabled()) {
			logger.debug("isValidForPost(String) - start"); //$NON-NLS-1$
		}

		boolean returnboolean = postCommands.containsKey(name);
		if (logger.isDebugEnabled()) {
			logger.debug("isValidForPost(String) - end"); //$NON-NLS-1$
		}
		return returnboolean;
	}
	...
	public static Command getCommand(final String name) {
		if (logger.isDebugEnabled()) {
			logger.debug("getCommand(String) - start"); //$NON-NLS-1$
		}

		try {
			Command returnCommand = Command.valueOf(name);
			if (logger.isDebugEnabled()) {
				logger.debug("getCommand(String) - end"); //$NON-NLS-1$
			}
			return returnCommand;
		} catch (Exception e) {
			logger.error("getCommand(String)", e); //$NON-NLS-1$
