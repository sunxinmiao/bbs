...
public class TokenBasedRememberMeServices extends AbstractRememberMeServices {
	 ...
	 */
	private static final Logger logger = Logger.getLogger(TokenBasedRememberMeServices.class);
	...
	protected UserDetails processAutoLoginCookie(String[] cookieTokens,
		...
		if (logger.isDebugEnabled()) {
			logger.debug("processAutoLoginCookie(String[], HttpServletRequest, HttpServletResponse) - start"); //$NON-NLS-1$
		}
		...
		try {
			...
			logger.error("processAutoLoginCookie(String[], HttpServletRequest, HttpServletResponse)", nfe); //$NON-NLS-1$
		...
		if (logger.isDebugEnabled()) {
			logger.debug("processAutoLoginCookie(String[], HttpServletRequest, HttpServletResponse) - end"); //$NON-NLS-1$
		}
		return user;
	}
	...
	protected String makeTokenSignature(long tokenExpiryTime, String username,
		...
		if (logger.isDebugEnabled()) {
			logger.debug("makeTokenSignature(long, String, String, Long) - start"); //$NON-NLS-1$
		}

		String returnString = DigestUtils.md5Hex(username + ":" + tokenExpiryTime + ":" + password + ":" + getKey() + ":" + id);
		if (logger.isDebugEnabled()) {
			logger.debug("makeTokenSignature(long, String, String, Long) - end"); //$NON-NLS-1$
		}
		return returnString;
	}
	...
	protected boolean isTokenExpired(long tokenExpiryTime) {
		if (logger.isDebugEnabled()) {
			logger.debug("isTokenExpired(long) - start"); //$NON-NLS-1$
		}

		boolean returnboolean = tokenExpiryTime < System.currentTimeMillis();
		if (logger.isDebugEnabled()) {
			logger.debug("isTokenExpired(long) - end"); //$NON-NLS-1$
		}
	...
	public boolean onLoginSuccess(HttpServletRequest request,
		...
		if (logger.isDebugEnabled()) {
			logger.debug("onLoginSuccess(HttpServletRequest, HttpServletResponse, UserDetails) - start"); //$NON-NLS-1$
		}
		...
		if (!StringUtils.hasLength(username)
			...
			if (logger.isDebugEnabled()) {
				logger.debug("onLoginSuccess(HttpServletRequest, HttpServletResponse, UserDetails) - end"); //$NON-NLS-1$
			}
			return false;
		}
	...
	protected int calculateLoginLifetime(HttpServletRequest request,
			UserDetails user) {
		if (logger.isDebugEnabled()) {
			logger.debug("calculateLoginLifetime(HttpServletRequest, UserDetails) - start"); //$NON-NLS-1$
		}

		int returnint = getTokenValiditySeconds();
		if (logger.isDebugEnabled()) {
			logger.debug("calculateLoginLifetime(HttpServletRequest, UserDetails) - end"); //$NON-NLS-1$
		}
		return returnint;
	}
}
