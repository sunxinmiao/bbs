...
public abstract class ImageUtils {
	...
	public static boolean isValidImageExt(String ext) {
		if (logger.isDebugEnabled()) {
			logger.debug("isValidImageExt(String) - start"); //$NON-NLS-1$
		}
		...
		for (String s : IMAGE_EXT) {
			if (s.equalsIgnoreCase(ext)) {
				if (logger.isDebugEnabled()) {
					logger.debug("isValidImageExt(String) - end"); //$NON-NLS-1$
				}
				return true;
			}
		}
	...
	public static boolean isImage(final InputStream in) {
		if (logger.isDebugEnabled()) {
			logger.debug("isImage(InputStream) - start"); //$NON-NLS-1$
		}

		ImageInfo ii = new ImageInfo();
		ii.setInput(in);
		boolean returnboolean = ii.check();
		if (logger.isDebugEnabled()) {
			logger.debug("isImage(InputStream) - end"); //$NON-NLS-1$
		}
	...
	public static Position markPosition(int width, int height, int p,
		...
		if (logger.isDebugEnabled()) {
			logger.debug("markPosition(int, int, int, int, int) - start"); //$NON-NLS-1$
		}
		...
		if (logger.isDebugEnabled()) {
			logger.debug("markPosition(int, int, int, int, int) - end"); //$NON-NLS-1$
		}
