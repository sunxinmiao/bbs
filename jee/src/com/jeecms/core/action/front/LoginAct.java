...
public class LoginAct {
	 ...
	 */
	private static final Logger logger = Logger.getLogger(LoginAct.class);
	...
	public String input(HttpServletRequest request, ModelMap model) {
		if (logger.isDebugEnabled()) {
			logger.debug("input(HttpServletRequest, ModelMap) - start"); //$NON-NLS-1$
		}
		...
		if (authId != null) {
			...
			if (auth != null) {
				...
				if (view != null) {
					if (logger.isDebugEnabled()) {
						logger.debug("input(HttpServletRequest, ModelMap) - end"); //$NON-NLS-1$
					}
					return view;
				} else {
	...
	public String submit(String username, String password, String processUrl,
		...
		if (logger.isDebugEnabled()) {
			logger.debug("submit(String, String, String, String, String, HttpServletRequest, HttpServletResponse, ModelMap) - start"); //$NON-NLS-1$
		}
		...
		if (!errors.hasErrors()) {
			try {
				...
				if (view != null) {
					if (logger.isDebugEnabled()) {
						logger.debug("submit(String, String, String, String, String, HttpServletRequest, HttpServletResponse, ModelMap) - end"); //$NON-NLS-1$
					}
					return view;
				} else {
					model.addAttribute("auth", auth);
				...
				logger.error("submit(String, String, String, String, String, HttpServletRequest, HttpServletResponse, ModelMap)", e); //$NON-NLS-1$
	...
	private WebErrors validateSubmit(String username, String password,
		...
		if (logger.isDebugEnabled()) {
			logger.debug("validateSubmit(String, String, HttpServletRequest) - start"); //$NON-NLS-1$
		}

		WebErrors errors = WebErrors.create(request);
		if (errors.ifOutOfLength(username, "username", 3, 100)) {
			if (logger.isDebugEnabled()) {
				logger.debug("validateSubmit(String, String, HttpServletRequest) - end"); //$NON-NLS-1$
			}
			return errors;
		}
