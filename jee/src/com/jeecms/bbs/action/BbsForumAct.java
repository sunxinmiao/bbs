...
public class BbsForumAct {
	...
	public String save(BbsForum bean, Integer categoryId, Integer[] views,
			...
			ModelMap model) {
		if (logger.isDebugEnabled()) {
			logger.debug("save(BbsForum, Integer, Integer[], Integer[], Integer[], HttpServletRequest, ModelMap) - start"); //$NON-NLS-1$
		}

		CmsSite site = CmsUtils.getSite(request);
		bean = manager.save(bean, categoryId, site, views, topics, replies);
		logger.info("save BbsForum id="+ bean.getId());
	...
	public String update(BbsForum bean, Integer categoryId, Integer[] views,
		...
		if (logger.isDebugEnabled()) {
			logger.debug("update(BbsForum, Integer, Integer[], Integer[], Integer[], Integer, HttpServletRequest, ModelMap) - start"); //$NON-NLS-1$
		}

		CmsSite site = CmsUtils.getSite(request);
		bean = manager.update(bean, categoryId, site, views, topics, replies);
		logger.info("update BbsForum id="+ bean.getId());
	...
	public String priorityUpdate(Integer[] ids, Integer[] prioritys,
		...
		if (logger.isDebugEnabled()) {
			logger.debug("priorityUpdate(Integer[], Integer[], Integer, HttpServletRequest, ModelMap) - start"); //$NON-NLS-1$
		}

		if (ids == null || ids.length <= 0) {
			if (logger.isDebugEnabled()) {
				logger.debug("priorityUpdate(Integer[], Integer[], Integer, HttpServletRequest, ModelMap) - end"); //$NON-NLS-1$
			}
			return "redirect:v_list.do";
		}
		CmsSite site = CmsUtils.getSite(request);
