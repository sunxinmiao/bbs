...
public abstract class BaseBbsMemberMagic  implements Serializable {
	 ...
	 */
	private static final Logger logger = Logger.getLogger(BaseBbsMemberMagic.class);
	...
	protected void initialize () {
		if (logger.isDebugEnabled()) {
			logger.debug("initialize() - start"); //$NON-NLS-1$
		}

		this.setNum(0);

		if (logger.isDebugEnabled()) {
			logger.debug("initialize() - end"); //$NON-NLS-1$
		}
	}
